...
static int  umass_init_device(MSC_T *msc)
{
    ...
    for (lun = 0; lun <= msc->max_lun; lun++)
    {
        ...
        {
            ret = msc_inquiry(try_msc);
            if (ret == 0)
                break;

            msc_debug_msg("msc_inquiry failed!!  %d\n", ret);
...
disk_found:
        ...
        for (retries = 0; retries < 3; retries++)
        {
            ...
            scsi_buff = (uint8_t *)((uint32_t)try_msc->scsi_buff | NON_CACHE_MASK);
            ret = run_scsi_command(try_msc, scsi_buff, 8, 1, 100);
            if (ret < 0)
            {
        ...
        try_msc->uTotalSectorN = (scsi_buff[0] << 24) | (scsi_buff[1] << 16) |
